...
try{
	...
	pst=con.prepareStatement("select test_type,state_code,centre,centre_code from test_city_centre order by test_type, centre");
	rs=pst.executeQuery();
	...
	function removerr(obj) { $(obj).removeClass("error_input"); }
		...
		function saveApp() {
			...
			// This line shows the red color ... 
				postRequest("SubmitArtsApplicationResult.jsp","Y");
			} else 
			{
...
/* 			if(document.getElementById("signpreview").innerHTML.indexOf("img") == -1) {
		...
		document.getElementById("appfee_flag").value=onlinePayFlag;
			...
			var validateform=$("#formPersonal").validate({
				onfocusout: false,
				invalidHandler: function(form, validator) {
					var errors = validator.numberOfInvalids();
					alert(errors);
					if (errors) {                    
						alert(validator.errorList[0].element.name);
						validator.errorList[0].element.focus();
					}
				} 
			});
			...
			if( $("#formPersonal").valid() ) {
				...
				/* if(sel_course.trim()=="BA" || sel_course.trim()=="B.Sc" || sel_course.trim()=="B.Com" || sel_course.trim()=="M.Sc"){
				...
				postRequest("SubmitArtsApplicationResult.jsp","N");
		...
		function approveApp() {
			postRequest2("ApproveArtsApplication.jsp","A");
		}
		function rejectApp() {
			postRequest2("ApproveArtsApplication.jsp","R");
		}
		function payment() {
		...
		function loadCourseList() {
			var fullTime=[ "MA", "M.Sc", "MBA", "MCA", "M.Com", "M.Phil"];
			var partTime=[ "MA", "M.Sc", "MBA", "M.Phil"];
      ...
      /* function loadtestcentre(obj){
		...
		function loadCourseData(obj) {
			...
			if(sel_course.trim()=="MCA" || sel_course.trim()=="MBA" || sel_course.trim()=="BBA" || sel_course.trim()=="BCA" || sel_course.trim()=="M.Phil") {
		...
		function loadCourses(val, course_type) {
			...
			for(i=0; i<arrCourses.length; i++) {
				...
				if( (val.trim()==arrCourses[i][0].trim()) && (course_type.trim()==arrCourses[i][3].trim()) ) {
					ch1.add(new Option(arrCourses[i][2], arrCourses[i][2]));// Changed (new Option(arrCourses[i][2], arrCourses[i][1]) to
					ch2.add(new Option(arrCourses[i][2], arrCourses[i][2]));// (new Option(arrCourses[i][2], arrCourses[i][2]) for choice name selection
					ch3.add(new Option(arrCourses[i][2], arrCourses[i][2]));// 1-code, 2-name. Done by Alex on 2.4.2015
				}
			}
		}
...
<body >
	...
	<input type='hidden' id="city_centre" name="city_centre" value="Bharath">
	<input type='hidden' id="exam_pref" name="exam_pref" value="Online">
	...
	<tr>
		...
		<td><input type="text" class="required" id="first_name" name="first_name"  value='<%=first_name%>' size=30 maxlength=30 class="" title="As it appears in your 10th/12th Standard Mark sheet" /></td>
...
<tr><td colspan=2 class="button navy gradient">Academics</td></tr>
	...
	<table width="90%">
		<tr>
			...
			<td class="button1 navy gradient" width='10%'>Marks/CGPA</td>
...
<!--    <tr>
		...
		-->
		<!-- Updated hostel and Transport by Alex-->
	<tr>
		<td>Hostel</td>
		<td>
		<select id="hostel_reqd" onChange='javascript:blockTransport()'>
			<option value="">Select</option>
...
//Added by Alexpandiyan on 23.3.15
	...
	<tr>
		...
		<td>
		<select id="transport_reqd" onChange='javascript:blockHostel()'>
			<option value="">Select</option>
			<option value="Y">Yes</option>
			<option value="N">No</option>
		</select> 
	...
	<!-- <tr>
		...
		<td>
 <select id="exam_pref" name="exam_pref" STYLE="width: 210px" onChange='javascript:loadCentre()'>
			<option value="">Select</option>
...
var getCentre=[];
var testType=[];
var testLocationCode=[];
var testLocation=[];


var getExam=document.getElementById("exam_pref").value;

<% for(int i=0;i<altest_type.size();i++) { %>
testType.push("<%= altest_type.get(i)%>")
<% } %>

<% for(int i=0;i<alCentre_code.size();i++) { %>
testLocationCode.push("<%= alCentre_code.get(i)%>")
<% } %>

<% for(int i=0;i<alCentre.size();i++) { %>
testLocation.push("<%= alCentre.get(i)%>")
<% } %>

for(var i=0;i<testType.length;i++)
{
if(testType[i]==getExam)
{
	getCentreCode.push(testLocationCode[i]);
	getCentre.push(testLocation[i]);
}
}
document.formPersonal.city_centre.options.length=0
for(var i=0;i<=getCentreCode.length;i++)
{
	document.formPersonal.city_centre.options[i]=new Option(getCentre[i],getCentre[i],false,false); // new Option(getCentre[i],getCentreCode[i],false,false) to new Option(getCentre[i],getCentre[i],false,false) by Alex on 2.4.2015
}
}
	...
	<tr><td colspan=2>&nbsp;</td></tr>
	<tr><td colspan=2>Application FEE is <strong>Rs.500</strong>. You can either pay the fee Online or by Cash / DD directly in the University.</td></tr>
	<tr><td colspan=2>&nbsp;</td></tr>
	<input name="appfee_flag" id="appfee_flag" type="hidden" value="S"/>
	
					<tr>
					<td colspan=2>* Your Application will be accepted only upon realisation of the money.  Till such time it will not be considered for enrolment.
					</td>
					</tr>
...
<tr><td colspan=2>&nbsp;</td></tr>
	...
	<tr>
		<td colspan=2>
			...
			<input type="hidden" id="student_id" name="student_id" value="<%=userID%>">
			Upload Signature&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="file" class="required" name="updateSignFile" size="20" value="Upload Signature" id="updateSignFile" class='rounded-corners' />
			</form>
	...
	<tr>
				<td colspan=2 align=right>
					...
					<!--  <input type="button" class="clickButton" value="Submit Application" onClick="javascript:submitApp('O')"/>   &nbsp;&nbsp;&nbsp; -->
							...
							<%} else { %>
						<!--  		<input type="button" class="clickButton" value="Submit Application" onClick="javascript:submitApp('O')"/>&nbsp;&nbsp;&nbsp;  -->
							 <input type="button" class="clickButton" value="Submit Application & Pay later" onClick='javascript:saveApp()'/>&nbsp;&nbsp;&nbsp; 
								 <input type="button" class="clickButton" value="Submit Application & Pay Online" onClick="javascript:submitApp('O')"/> 
							<% } 
								}
							%>
