...
if(flag.intern()=="I".intern()) {
		...
		/*
		 pst=con.prepareStatement("select max(appn_no) from application where mkr_id=?");
		 pst.setString(1,mkr_id);
		 ...
		 */
		 ...
		 /* Updated by Alexpandiyan 23.3.15. */
		 
			pst=con.prepareStatement("select max(appn_no)+1 from application"); 
			rs=pst.executeQuery();
			if(rs.next())
			appn_no=rs.getString(1);
			pst=null;
	...
	if(flag.trim().intern()=="U".intern()) {
		pst.setString(85,appn_no);
	}
	pst.setString(86,exam_pref);
	pst.setString(87,city_centre);
	
	System.out.println("appn_no"+appn_no);
	f_appn_no=(f_appn_no+appn_no);
	System.out.println("f_appn_no"+f_appn_no);
	pst.setString(88,f_appn_no);
	pst.executeUpdate();
	} 
	...
	mesg="0000"; // "Your Application has been Submitted Successfully. Application Number is "+appn_no;
...
if(appfee_flag.intern()=="O".intern()) {
	...
	String txnNo=System.currentTimeMillis()+""; //request.getParameter("txn_id");
	String str="BHARTUNIV|"+txnNo+"|NA|900.00|NA|NA|NA|INR|NA|R|bhartuniv|NA|NA|F|"+appn_no+"|APPLN|"+first_name+"|NA|NA|NA|NA|http://bharathuniverp.com/admissionWeb2/PaymentStatus.jsp";
	//String str="BHARTUNIV|"+txnNo+"|NA|2.00|NA|NA|NA|INR|NA|R|bhartuniv|NA|NA|NA|"+appn_no+"|APP|"+name+"|NA|NA|NA|NA|http://localhost:8080/admission/PaymentStatus.jsp";
	String commonstr="5v2V1QMwY7uv";
	String cksum=CheckSumUtil.checkSumSHA256 (str+"|"+commonstr);
	String fullStr=str+"|"+cksum;
	System.out.println("Request to Billdesk "+fullStr);	
