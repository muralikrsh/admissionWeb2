...
try{
	...
	{	
		...
		pst=con.prepareStatement("select count(1) from application where mobile_no=?");
		...
		if(count==0)
		{
			...
			pst=con.prepareStatement("SELECT CASE WHEN MAX(appn_no) IS NULL THEN 300001 ELSE MAX(appn_no)+1 END AS appn_no FROM application");
			rs=pst.executeQuery();
			if(rs.next())
			{
	...
	pst.setString(89,strFullAppNumber);
	} 
	
	if(flag.trim().intern()=="U".intern()) {
		pst.setString(85,strFullAppNumber);
	}
	pst.executeUpdate();
	} 
	...
	mesg="0000"; // "Your Application has been Submitted Successfully. Application Number is "+strFullAppNumber;
...
if(appfee_flag.intern()=="O".intern()) {
	...
	String str="BHARTUNIV|"+txnNo+"|NA|900.00|NA|NA|NA|INR|NA|R|bhartuniv|NA|NA|F|"+strFullAppNumber+"|APPLN|"+first_name+"|NA|NA|NA|NA|http://bharathuniverp.com/admissionWeb2/PaymentStatus.jsp";
	...
	mesg+="~"+strFullAppNumber+"~"+fullStr;
 } else {
	mesg+="~"+strFullAppNumber;
}
