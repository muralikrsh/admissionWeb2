...
//alert("Check3");
			...
			if( $("#formPersonal").valid() ) {
				...
				if( ! validateMY10(sslc_mon_yop) ) {
				displayAlert("Maximum Year of Passing is 2014 for 10th");
					return;
				}
			//alert("Check5");
				if( ! validateMY(hsc_mon_yop) ) {
				displayAlert("Maximum Year of Passing is 2016");
					return;
				}
				<% } %>
				...
				//validateMYPG10
				if( ! validateMYPG10(sslc_mon_yop) ) {
				displayAlert("Maximum Year of Passing is 2008 for 10th");
					return;
				}
				
				if( ! validateMYPG(hsc_mon_yop) ) {
				displayAlert("Maximum Year of Passing is 2013");
					return;
				}
				...
				postRequest("SubmitApplicationResult.jsp");
			} else {
		...
		if(inputField.value.trim()=="")
				return false;
        if(inputField.value.trim()<'2010')
		{
			var isValid = /^([2][0][0]?[0-9])$/.test(inputField.value);
		}
		else if(inputField.value.trim()>'2009')
		{
		...
		function validateMYPG10(inputField) {
		if(inputField.value.trim()=="")
				return false;
		var isValid = /^([2][0][0]?[0-8])$/.test(inputField.value);
        if (isValid) {
            inputField.style.backgroundColor = '#fff';
        } else {
            inputField.style.backgroundColor = '#fba';
		...
		{
			var isValid = /^([2][0]?[0-1]?[0-6])$/.test(inputField.value);
		}
        if (isValid) {
            inputField.style.backgroundColor = '#fff';
        } else {
		...
		function validateMYPG(inputField) {
		if(inputField.value.trim()=="")
				return false;
        if(inputField.value.trim()<'2010')
		{
		...
		{
			var isValid = /^([2][0]?[0-1]?[0-3])$/.test(inputField.value);
		}
        if (isValid) {
            inputField.style.backgroundColor = '#fff';
        } else {
...
<body onload="disableCourse()">
	...
	<tr>
		<td>Date of Birth</td>
		<td align=left nowrap>
		<input type="text" name="dob" value="<%=dob %>" readonly="readonly" id="dob" STYLE="width:210px"/>
		</td>
	</tr>
	...
	<select id="course" name="course" class="required" onChange='javascript:loadCourseData(this)' STYLE="width: 210px">
		<option value="">Select</option>
	<% if (category.intern()=="UG".intern()) {
			if( strDegree.intern()=="Engr".intern()){
	%>
		<option value="B.Tech">B.Tech</option>
	<%} else if( strDegree.intern()=="Arch".intern()){%>
		<option value="B.Arch">B.Arch</option>
	<%} else if( strDegree.intern()=="Arts".intern()){%>
	...
	else if (category.intern()=="PG".intern()) {
			if( strDegree.intern()=="Engr".intern()){%>
	...
	<%} else if( strDegree.intern()=="Mgmt".intern()){%>
	...
	<tr>
		<td>Valid Score</td>
		<td>
		<input type="text" id="valid_score_of" name="valid_score_of" value="<%=valid_score_of%>" STYLE="width: 210px">
		</td>
	</tr>
	<% } else { %>
...
<tr><td colspan=2 class="button navy gradient">Miscellaneous</td></tr>
	<tr>
		<td>Hostel</td>
		<td>
		<select id="hostel_reqd" value="<%=hostel_reqd %>" onChange='javascript:blockTransport()' STYLE="width:210px">
			<option value="">Select</option>
	...
	<tr>
		...
		<td>
		<select id="transport_reqd" value="<%=transport_reqd %>" onChange='javascript:blockHostel()' STYLE="width:210px">
			<option value="">Select</option>
			<option value="Yes">Yes</option>
			<option value="No">No</option>
		</select>
...
<tr><td colspan=2>&nbsp;</td></tr>
	...
	<tr>
		<td colspan=2>
			...
			Upload Photo&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="file" class="required" name="updatePhotoFile" size="20" value="Upload Photo" id="updatePhotoFile" class='rounded-corners' />* Maximum File size 100kb.
			</form>
	...
	<tr>
		<td colspan=2>
			...
			<input type="hidden" id="student_id" name="student_id" value="<%=userID%>">
			Upload Signature&nbsp;&nbsp;&nbsp;<input type="file" class="required" name="updateSignFile" size="20" value="Upload Signature" id="updateSignFile" class='rounded-corners' /> * Maximum File size 20kb.
			</form>
	...
	<tr>
				<td colspan=2 align=right>
					...
					<!--  <input type="button" class="clickButton" value="Submit Application" onClick="javascript:submitApp('O')"/>   &nbsp;&nbsp;&nbsp; -->
							...
							<%} else { %>
							<!-- <input type="button" class="clickButton" value="Submit Application" onClick="javascript:submitApp('O')"/>&nbsp;&nbsp;&nbsp;  -->
								 <input type="button" class="clickButton" value="Save Application" onClick="javascript:submitApp('S')"/>&nbsp;&nbsp;&nbsp; 
							<!-- <input type="button" class="clickButton" value="Submit Application & Pay Online" onClick="javascript:submitApp('O')"/> -->
							<% } 
								}
							%>
