...
try {
	...
	if(flag!=null) {
		...
		System.out.println(from_date+":::"+to_date+"::::"+f_appn_no+"::::"+course+"::::"+first_name+"::::"+last_name+"::::"+state+"....."+city+"....."+appn_status);
		//+"...."+opt_sub
		String where_clause="";
		System.out.println("1 ->"+where_clause);
		if(f_appn_no.intern()!="".intern()) {
			where_clause=" binary f_appn_no=?";
		}
		...
		*/
		...
		if(where_clause.intern()!="".intern())  {
			...
			if(strRole.intern()=="OFFLINE".intern()) {
				where_clause=" where mkr_id = '"+strId+"'";
			}
		}
	
		// Removed religion, community, sslc_marks
//		System.out.println("select f_appn_no, first_name, ifnull(last_name,'') last_name, city, email_id, mobile_no, course, date_format(submission_date,'%d %b %Y'), IF(STRCMP(appn_status,'A')=0,'Approved',(if(STRCMP(appn_status,'R')=0,'Rejected','Submitted'))) from application "+where_clause+" limit 50");
		pst=con.prepareStatement("select f_appn_no, first_name, ifnull(last_name,'') last_name, city, email_id, mobile_no, course, date_format(submission_date,'%d %b %Y'), IF(STRCMP(appn_status,'A')=0,'Approved',(if(STRCMP(appn_status,'R')=0,'Rejected','Submitted'))) from application_manual "+where_clause+" "); //  limit 50 removed by Alex
		int ctr=0;

		if(where_clause.indexOf("f_appn_no")!= -1) {
			pst.setString(++ctr, f_appn_no);
		}
		...
		*/
		...
		while(rs.next()) {
			...
			alSub.add(rs.getString(1)); // f_appn_no
    ...
    <script src="Scripts/CustomScript.js" type="text/javascript"></script>
		...
		$(function() {
			...
			<%
				...
				%>
					aDataSet[<%=i%>][0]='<%=alSub.get(0)%>&nbsp;<a href="SubmitManualApplication2Edit.jsp?f_appn_no=<%=alSub.get(0)%>&flag=U">EDIT</a>';
				<%
					} else {
					%>
					aDataSet[<%=i%>][0]='<%=alSub.get(0)%>&nbsp;<a href="SubmitManualApplication2Edit.jsp?f_appn_no=<%=alSub.get(0)%>&flag=U">EDIT</a>';
			...
			function getSMSData() {
				location.href='ManualApplicationList.jsp?from_date='+document.getElementById("from_date").value+
				'&to_date='+document.getElementById("to_date").value+
				'&f_appn_no='+document.getElementById("f_appn_no").value+
			...
			function exportExcel() {
				strURL="servlet/ManualApplicationExcel";
...
<body>
	...
	<tr>
	 ...
	 <td>Appn No</td>
	<td><input type="text" name="f_appn_no" value="<%=f_appn_no%>"  id="f_appn_no"/></td>
	 </tr>
	 </tr>
