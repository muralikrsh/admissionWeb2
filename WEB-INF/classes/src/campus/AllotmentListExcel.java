...
public class AllotmentListExcel extends HttpServlet
{
  ...
  public void service(HttpServletRequest paramHttpServletRequest, HttpServletResponse paramHttpServletResponse) throws ServletException, IOException
  {
	    ...
	    try
	    {
	      ...
	      pst = localConnection.prepareStatement("SELECT full_adm_no AS Allotment_No, stu_name AS Student_Name, par_name AS Parent_Name, course AS Course, branch AS Branch, tution_fee AS Tuition_Fee, mobile AS Mobile, email AS Email, pay_type_1 AS Paid_By, cash_ref_no_1 AS Cash_Ref_No, dd_no_1 AS DD_No, amount_1 AS Paid_Amount FROM student_admission WHERE full_adm_no=1");
